...
class TeleopTurtleBot(Node):    # Define una clase llamada 'TeleopTurtleBot' que hereda de la clase 'Node'
        ...
        def __init__(self):    # Define un método llamado '__init__' que se ejecutará cuando se cree una instancia de la clase
                super().__init__('robot_teleop')    # Llama al método '__init__' de la clase 'Node' y le da un nombre al nodo
        ...
        def on_press(self, key): 
                ...
                self.get_logger().info('Sending - Linear Velocity : %f, Angular Velocity : %f' % (self.twist_msg_.linear.x, self.twist_msg_.angular.z))
                self.publisher_.publish(self.twist_msg_)    # Publica el mensaje 'twist_msg_' en el tópico '/turtlebot_cmdVel'
